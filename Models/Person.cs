    ...
    public class Person:BaseClass 
        ...
        public void MapIntoIOSJson(string PathPrefix, bool HasDetail = false)
        {
             ...
             if (HasDetail)
             {
                  ...
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_DOB", Convert.ToDateTime(dob).ToString("MM/dd/yyyy"));
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_WeightLbs", weight);
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_WeightKgs", ( (Utilities.IsNumeric(weight) ? double.Parse(weight):0) * 0.45).ToString());
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_Gender", is_male + "" == "" ? null : is_male + "" == "1" || (is_male+"").ToLower()  == "true" ? "Male" : "Female");
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_Age", age);
                  JsonMaker.UpdateJsonValue(DetailPrefix + "_SSN", ss);

                  string SelectQuery = "(select pn.* from person p inner join person_phones pp on p.id=pp.person_id inner join phone_number pn on pn.id=pp.phone_id where person_id = '" + this.id + "' order by is_primary desc)";
             ...
             else
             {
                  ...
                  for (int i = 1; i <= 2 && i <= PhonesList.Count; i++)
                  {
                       JsonMaker.UpdateJsonValue(PathPrefix + "_Phone" + i.ToString(), PhonesList[i - 1].number);
                  }
             }
        ...
        public Person(object JsonData,string PathPrefix, bool HasDetail=false)  
        {
          ...
          if (HasDetail)
          {
               ...
               DateTime dateValue;
               if (!DateTime.TryParse(dob == null ? "" : dob.ToString(), out dateValue))
                    dob = null;
               weight = JsonMaker.GetIOSJsonExtract("$." + DetailPrefix + "_WeightLbs", JsonData);
               Decimal decWeight;
               if (!Decimal.TryParse(weight == null ? "" : weight.ToString(), out decWeight))
                    weight = null;
               is_male = JsonMaker.GetIOSJsonExtract("$." + DetailPrefix + "_Gender", JsonData) == "Male" ? "1" : "0";
               int age;
               int.TryParse(string.Join("", (JsonMaker.GetIOSJsonExtract("$." + DetailPrefix + "_Age", JsonData) ??"").ToCharArray().Where(Char.IsDigit)), out age);
          ...
          if (HasDetail)
          {
               person_phones phone = new person_phones(JsonData, DetailPrefix + "_Phone1", "1");
               ...
               phone = new person_phones(JsonData, DetailPrefix + "_Phone2", "0");
               if (phone.phone_id != null)
               {
                    phone.person_id = id;
                    phone.HandleRecord();
