...
namespace WebApiPoster.Models
    {
    public class Ems_run:BaseClass 
        {
        ...
        public void HandleWithUserName(string DriverName, string EmtName, string agency_id)
        {
             this.status = "1";
             this.HandleRecord();
             User_Login user_login = new User_Login();
             user_login.UpdateUsingUserName((HttpContext.Current.Session["bus_id"] ?? "").ToString(), DriverName, EmtName, agency_id);
        ...
        public void HandleWithUserName(Members DriverMember, Members EmtMember, string agency_id)
        {
             this.status = "1";
             this.HandleRecord();
             User_Login user_login = new User_Login();
             user_login.UpdateUsingUserName((HttpContext.Current.Session["bus_id"] ?? "").ToString(), DriverMember, EmtMember, agency_id);
             _form_800_id = user_login.form800_id;
             Run_Logins run_login = new Run_Logins();
             run_login.user_login = user_login.id;
             run_login.ems_run = this.id;
