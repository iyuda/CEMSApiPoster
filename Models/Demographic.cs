...
namespace WebApiPoster.Models
    {
    public class Demographic : BaseClass
        {
             ...
             public void MapIntoIOSJson()
             {
                  try
                  {
                       if (pt_person_object!=null) pt_person_object.MapIntoIOSJson("$.Demographics.PatientInfo@PatientInfo",true);
                       if (emr_contact_person_object != null) emr_contact_person_object.MapIntoIOSJson("$.Demographics.EmergencyContact@EmergencyContact");
                       if (phy_contact_person_object != null) phy_contact_person_object.MapIntoIOSJson("$.Demographics.PhysicianInfo@PhysicianInfo");
             ...
             public static string GetIDByNameAndDOB(string last_name, string dob,  out string pcr_id)
             {
                  using (MySqlConnection cn = new MySqlConnection(DbConnect.ConnectionString))
                  {
                       cn.Open();
                       string SqlString = "select concat(d.id,',', pcr.id)  as p_id from pcr_demographic d inner join pcr on pcr.pcr_demographic_id=d.id inner join person p on d.pt_person=p.id where p.last_name = '" + last_name + "' and dob = '"+ dob + "' order by d.utc_insert desc limit 1";
             ...
             public static string GetIDBySSNo(string SS, out string pcr_id)
             {
                  using (MySqlConnection cn = new MySqlConnection(DbConnect.ConnectionString))
             {
                  cn.Open();
                  string SqlString = "select concat(d.id,',', pcr.id)  as p_id from pcr_demographic d inner join pcr on pcr.pcr_demographic_id=d.id inner join person p on d.pt_person=p.id where p.ss = '" + SS + "' order by d.utc_insert desc limit 1";
                  MySqlCommand cmd = new MySqlCommand(SqlString, cn);
                  string rv = cmd.ExecuteScalar()+"";
                  if (String.IsNullOrEmpty(rv))
                  {
