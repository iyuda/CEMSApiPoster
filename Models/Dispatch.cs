...
namespace WebApiPoster.Models
    ...
    public class Dispatch : BaseClass
        ...
        public Dispatch(string id)
            ...
            if (!string.IsNullOrEmpty(this.facility_id)) facility_object = new Business(facility_id, true);
            if (!string.IsNullOrEmpty(this.town_id)) town_object = new Town(town_id);
        ...
        public void MapIntoIOSJson()
          {
               try
               {
                    ...
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_Assigned", Convert.ToDateTime(assigned).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_EnRoute", Convert.ToDateTime(en_route_63).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_OnScene", Convert.ToDateTime(on_scene_84).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_PatientContact", Convert.ToDateTime(pt_contact).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_FromScene", Convert.ToDateTime(from_scene_82).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_AtDestination", Convert.ToDateTime(at_destination).ToString("HH:mm"));
                    JsonMaker.UpdateJsonValue("$.Dispatch.Timeline@Timeline_InService", Convert.ToDateTime(in_service).ToString("HH:mm"));
                    ...
                    if (this.town_object != null)
                    {
                         //Town town = new Town(this.town_id);
                         town_object.MapIntoIOSJson("$.Dispatch.DispatchInfo@DispatchInfo");
                    }
                    if (this.address_object != null)
                    {
                         //Address address = new Address(this.address_id,true);
                         address_object.MapIntoIOSJson("$.Dispatch.CallLocation@CallLocation");
                    }
                    if (this.facility_object != null)
                    {
                         //Business facility = new Business(this.facility_id,true);
                         facility_object.MapIntoIOSJson("$.Dispatch.CallLocation@CallLocation");
                    }
                    Inputs pcr_input = new Inputs(); pcr_input.MapDispatchIntoIOSJson(HttpContext.Current.Session["pcr_id"].ToString(), "aptRoom");
     ...
     public void MapFromIOSJson(object JsonData)
     {
          try
          {
               ...
               assigned=Utilities.ValidateTimeField(assigned);
               en_route_63 = Utilities.ValidateTimeField(en_route_63);
               on_scene_84 = Utilities.ValidateTimeField(on_scene_84);
               pt_contact = Utilities.ValidateTimeField(pt_contact);
               from_scene_82 = Utilities.ValidateTimeField(from_scene_82);
               at_destination = Utilities.ValidateTimeField(at_destination);
               in_service = Utilities.ValidateTimeField(in_service);
               ...
               if (!Utilities.IsNumeric(mileage_begin)) mileage_begin = "0";
//               mileage_begin += "~";
               mileage_end = JsonMaker.GetIOSJsonExtract("$.Mileage@Mileage_End", JsonData);
               if (!Utilities.IsNumeric(mileage_end)) mileage_end = "0";
  //             mileage_end += "~";
               ...
               if (address.address  != null)
               {
                    ...
                    address.HandleRecord();
                    this.address_id = address.id;
               }
               Business facility = new Business(JsonData, "$.CallLocation@CallLocation");
               if (facility.name != null)
               {
                    facility.HandleRecord();
                    //facility.InsertUpdateAction();
                    this.facility_id = facility.id;
               }
               Inputs pcr_input = new Inputs(); pcr_input.MapDispatchfromIOSJson(JsonData, HttpContext.Current.Session["pcr_id"].ToString(), "aptRoom");
        ...
        public Town town_object { get; set; }
