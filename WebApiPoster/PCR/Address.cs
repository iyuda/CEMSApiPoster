...
namespace WebApiPoster.PCR
{
    public class Address :PcrBase 
    {
             ...
             
        {
             ...
             string Zip=JsonMaker.GetIOSJsonExtract(PathPrefix + (PathPrefix.Contains("CallLocation")?"_ZipCode":"_Zip"), JsonData);
        ...
        public void HandleRecord()
         ...
         public string GetZipNameById(string id)
         {
              ...
              {
                   cn.Open();
                   string SqlString = "select zip_code from zip where id = '" + id + "'";
                   MySqlCommand cmd = new MySqlCommand(SqlString, cn);
                   string rv = (string)cmd.ExecuteScalar();
                   //rv = rv == null ? "" : rv.ToString();
                   return rv;
