...
{
     public class Pcr_Buttons : PcrBase
     {
          static Pcr_Buttons()
          {
               InitVitals();
          ...
          public Pcr_Buttons()
          {
               this.TableName = "pcr_buttons";
          }
          public Pcr_Buttons(string id)
               : base(id, "pcr_buttons")
          {

          }
          public Pcr_Buttons(string TableName, JsonInputSection PcrObj)
          {
          ...
          public static void InitVitals()
          {
               ...
               VitalsList.Add("Pulse.UTO", "UTO"); //new List<object> { false, "2_1" });
               VitalsList.Add("Pulse.PALP", "PALP");
          ...
          public void SaveVitalBySelection(object JsonData, string VitalNo, string Category, string Range, string pcr_id = "")
          {
               ...
               string ButtonIDSuffix = Pcr_Buttons.VitalsList.ContainsKey(VitalID) ? Pcr_Buttons.VitalsList[VitalID] : "";
               if (VitalNo!="1") {
                    string SectionID = ButtonIDSuffix.Contains("_") ? ButtonIDSuffix.Split('_')[0] : "";
                    string NewSectionID = Pcr_Buttons.VitalsSetConversionList.ContainsKey(SectionID) ? Pcr_Buttons.VitalsSetConversionList[SectionID] : "";
                    if (SectionID != "" && NewSectionID != "") ButtonIDSuffix = ButtonIDSuffix.Replace(SectionID, NewSectionID);
               }    
                
               if (Value != null) this.button_id = All_Buttons.GetIDByName(Value, VitalNo, ButtonIDSuffix.ToString());
               this.Clicked = "1";
               if (this.button_id != null)
               {
                    string DeletePattern = "Vitals" + VitalNo +"Set"+ButtonIDSuffix.ToString().Split('_')[0] + "_["+ Range +"]$";
                    DeletePrevSlections(pcr_id, DeletePattern);
                   // this.Delete(new string[] { "pcr_id", "button_id" }, new string[] { pcr_id, this.button_id.Split('_')[0]});
                    this.HandleRecord();
               }
                         
          }
          private void DeletePrevSlections(string pcr_id, string ButtonIDPattern)
          {
          ...
          public void SaveButton(object JsonData, string pcr_id = "")
          {
               try
               {
                    ...
                    if (str != null) this.button_id = All_Buttons.GetIDByName(str);
                    this.Clicked = "1";
                    if (this.button_id != null)
                         if (choice + "" == "on")
                              this.HandleRecord();
          ...
          public void SaveVitalByButton(object JsonData, string VitalNo, string Path, string pcr_id = "")
          {
               try
               {
                    ...
                    object ButtonIDSuffix = Pcr_Buttons.VitalsList.ContainsKey(VitalID) ? Pcr_Buttons.VitalsList[VitalID] : "";
                    string choice = JsonMaker.GetIOSJsonExtract("$.choice", JsonData);
                   
                    //bool IsVital = JsonToken.Path.StartsWith("Vitals_");

                    if (id != null) this.button_id = All_Buttons.GetIDByName(id, VitalNo, ButtonIDSuffix.ToString());
                    this.Clicked = "1";
                    if (this.button_id != null)
                         if (choice + "" == "on")
                              this.HandleRecord();
