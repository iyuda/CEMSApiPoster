...
namespace WebApiPoster.PCR
    ...
    public class Bus : PcrBase
        ...
        public static string GetBusIDByAgency(string agency_id)
        {
             using (MySqlConnection cn = new MySqlConnection(DbConnect.ConnectionString))
             {
                  cn.Open();
                  string SqlString = "select id from bus where bus_number = '' and agency_id = '" + agency_id + "'";
